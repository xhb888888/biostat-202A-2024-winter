---
title: "hw2"
author: "Hanbei Xiong"
format: pdf
editor: visual
---

# Problem 1

![](images/4681706478246_.pic.jpg)

**Answer:**

$$
\left\{
    \begin{array}{ll}
        V=5 & \text{if } 0<t < 3 \\
        V=2T & \text{if } t \geq 3
    \end{array}
\right.
$$

So the support of cdf of V with its support is:

$$
F_{V}(v) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } v < 5 \\
        \int_{0}^{3} \frac{1}{1.5}e^{-t/(1.5)}dt & \text{if } 5\leq v <6 \\
        \int_{0}^{3} \frac{1}{1.5}e^{-t/(1.5)}dt+\int_{3}^{v/2} \frac{1}{1.5}e^{-t/(1.5)}dt & \text{if } v \geq 6
    \end{array}
\right.
$$

# Problem 2

![](images/4691706478253_.pic.jpg)

**(a) Answer:**

Since $\lambda$ is positive, $f(x) = \frac{1}{2} \lambda e^{-\lambda |x|}$ is positive for all x.

$\int_{-\infty}^{\infty}f(t)=\int_{0}^{\infty}\frac{1}{2}\lambda e^{-\lambda t}+\int_{-\infty}^{0}\frac{1}{2}\lambda e^{\lambda t}=-\frac{1}{2} e^{-\lambda t}\Big|_0^\infty + \frac{1}{2} e^{\lambda t}\Big|_{-\infty}^{0}=\frac{1}{2}+\frac{1}{2}=1$

Both properties of pdf have satisfied. We can conclude that $f(x)$ is a pdf.

**(b) Answer:**

$P(X<t)=F_{X}(t)$

$$
F_{X}(t) = 
\left\{
    \begin{array}{ll}
        \int_{-\infty}^{t} \frac{1}{2} \lambda e^{\lambda x} dx & \text{if } t < 0 \\
        \int_{-\infty}^{0} \frac{1}{2} \lambda e^{\lambda x} dx+\int_{0}^{t} \frac{1}{2} \lambda e^{-\lambda x} dx & \text{if } t \geq 0
    \end{array}
\right.
$$

$$
F_{X}(t) = 
\left\{
    \begin{array}{ll}
        \frac{1}{2}e^{\lambda t} & \text{if } t < 0 \\
        \frac{1}{2}-\frac{1}{2} e^{-\lambda t}+\frac{1}{2} =1-\frac{1}{2} e^{-\lambda t}& \text{if } t \geq 0 
    \end{array}
\right.
$$

**(c) Answer:**

By break out the absolute value of X, we have range of X: $X>t$ or $X<-t$

$$
F_{X}(t) = 
\left\{
    \begin{array}{ll}
        \int_{t}^{-t} \frac{1}{2} \lambda e^{\lambda x} dx & \text{if } t < 0 \\
        \int_{-\infty}^{0} \frac{1}{2} \lambda e^{\lambda x} dx+\int_{0}^{t} \frac{1}{2} \lambda e^{-\lambda x} dx & \text{if } t \geq 0
    \end{array}
\right.
$$

# Problem 3

![](images/4701706478264_.pic.jpg)

**Answer:**

We can simply find the cdf of X:

$$
u(x)=F_{X}(t) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } t \leq 1 \\
        \int_{1}^{t} \frac{x-1}{2}  dx= \frac{t^2}{4} -\frac{1}{2}t+\frac{1}{4}& \text{if } 1 < t < 3 \\
        \int_{1}^{3} \frac{x-1}{2}  dx=1 & \text{if } t \geq 3
    \end{array}
\right.
$$

# Problem 4

![](images/4711706478272_.pic.jpg)

**Answer:**

Since X is uniform distribution, the pdf of X is:

$$
f_{X}(x) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } x < -1 \\
        \frac{1}{2} & \text{if } -1\leq x \leq 1 \\ 
        0 & \text{if } x > 1 
    \end{array}
\right.
$$ Then, the cdf of X is:

$$
F_{X}(x) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } x < -1 \\
        \frac{1}{2}x & \text{if } -1\leq x \leq 1 \\ 
        1 & \text{if } x > 1 
    \end{array}
\right.
$$

We can think \|X\| as a random variable Y, which means Y=\|X\|

When $-1\leq x<0$, Y=-X, so the cdf of Y is:$-\frac{1}{2}y$

When $0\leq x\leq1$, Y=X, so the cdf of Y is:$\frac{1}{2}y$

Therefore, the cdf of Y is:

$$
F_{Y}(y) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } y < -1 \\
        -\frac{1}{2}y & \text{if } -1\leq y \leq 0 \\
        \frac{1}{2}+\frac{1}{2}y & \text{if } 0\leq y \leq 1 \\
        1 & \text{if } y > 1 
    \end{array}
\right.
$$

# Problem 5

![](images/4721706478278_.pic.jpg)

# Problem 6

![](images/4731706478289_.pic.jpg)

**Answer:**

Based on the assumption, we have

$$
F_X(x)=P(X<x) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } x \leq 0 \\
        \frac{\pi x^2}{\pi}=x^2 & \text{if } 0<x<1 \\
        1& \text{if } x \geq 1 
    \end{array}
\right.
$$ Then, the pdf of X is:

$$
f(x) = 
\left\{
    \begin{array}{ll}
        2x & \text{if } 0<x<1 \\
        0& \text{else }  
    \end{array}
\right.
$$

Since $Y=X^3$, the cdf of Y is:

$$
F_Y(y)=P(Y<y) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } y \leq 0 \\
        y^{\frac{1}{3}} & \text{if } 0<y<1 \\
        1& \text{if } y \geq 1 
    \end{array}
\right.
$$

Then, the pdf of Y is:

$$
f(y) = 
\left\{
    \begin{array}{ll}
        \frac{1}{3}y^{-2/3} & \text{if } 0<y<1 \\
        0& \text{else }  
    \end{array}
\right.
$$

# Problem 7

![](images/4741706478296_.pic.jpg)

**Answer:**

Since pdf of X is $f(x)=\frac{|x|}{4}$, $0\leq |x|\leq 2$, we can convert the support of X by breaking the absolute value of X based on sign of x

Then, the pdf of X is:

$$
f(x) = 
\left\{
    \begin{array}{ll}
        0 & \text{if } x< -2 \\
        -\frac{x}{4}& \text{if } -2 \leq x \leq 0 \\
        \frac{x}{4}& \text{if } 0 \leq x \leq 2 \\
        0& \text{if } x>2
    \end{array}
\right.
$$

$1\leq |X| \leq 2$ can also be break parts:

If X is positive, we have $1\leq x \leq 2$

If X is negative, we have $-2\leq x \leq -1$

Then,

$P(1\leq |X| \leq 2)=P(1\leq x \leq 2)+P(-2\leq x \leq -1)=\int_{1}^{2}\frac{x}{4}dx-\int_{-2}^{-1}\frac{x}{4}dx=\frac{3}{4}$
